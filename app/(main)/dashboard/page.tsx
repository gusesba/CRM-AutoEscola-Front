  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-card p-4 rounded-xl border">
        <Select label="Sede" value={sede} onChange={setSede} options={sedes} />
        <Select
          label="Vendedor"
          value={vendedor}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <DashboardCard
          ...
          valor={resumo?.totalMatriculas ?? 0}
          icon={<CheckCircle />}
          destaque
        />
        <DashboardCard
          titulo="Leads Abertos"
          valor={resumo?.leadsAbertos ?? 0}
          icon={<Users />}
        />
        <DashboardCard
          titulo="Leads Sem Sucesso"
          valor={resumo?.leadsSemSucesso ?? 0}
          icon={<XCircle />}
          danger
        />
        <DashboardCard
      ...
      <div className="bg-card border rounded-xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {comparativo.map((v) => {
              const conversao =
                v.leads > 0 ? ((v.matriculas / v.leads) * 100).toFixed(1) : "0";
